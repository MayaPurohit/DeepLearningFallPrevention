...
def train(config):
    ...
    for k in range(len(config['user_num'])):
        ...
        if config['test_type'] == "individual":
            fig_conf.savefig(os.path.join(config['save_dir'], f"Confusion_Matrix (K-Fold) User {config['user_num'][k]}"))
    ...
    print(f"Test Accuracy for Each User: {complete_accs/100}")
...
if __name__ == "__main__":
    ...
    config = {
        ...
        'save_dir': "/content/drive/MyDrive/DeepLearningFallDetection/CNN_Models/Personal_Model/Model2-AddActivation/2d-stack4-200Feat",  # Directory specific to the model being tested
